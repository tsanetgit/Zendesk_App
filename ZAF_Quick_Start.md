...
> This trigger fires when the `tsanet_sla_breached` tag is added by the ZAF background poller while an agent has Zendesk open. (TSANet also enforces the acknowledgment SLA on its own servers regardless of any Zendesk-side alerting.)
