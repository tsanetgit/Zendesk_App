...
that never worked, and it blocked a member onboarding in July 2026. This is documented too: Zendesk's getting-started tutorial says the registration response contains the OAuth client and to "save the response in a safe place" because "later, you can use the client id to obtain a ZIS OAuth token". We invented a step the vendor docs never had.
...
- **ZIS integration names are GLOBALLY UNIQUE across all of Zendesk, and this is documented.** [Zendesk's own tutorial](https://developer.zendesk.com/documentation/integration-services/zis-tutorials/getting-started/building-your-first-zis-integration/) says it plainly: "The name you choose has to be globally unique and can be up to 64 characters long", with the charset "lower-case letters (a-z), numbers, hyphens (-), and underscores (_)". Register a name and no other Zendesk customer can ever use it. `POST /api/services/zis/registry/{name}` returns `400 {"message":"the integration: <name> is not available for upsert by this account"}` when the name is already taken — by design, not a permissions fault, and not something to debug on the member's side. Consequence for this connector: `tsanet_connect` is claimed, so every member needs their own name, and the bundle + `deploy.js` had it hardcoded until `tsanetgit/Zendesk_App#174`. **Read the vendor tutorial before probing runtime behaviour** — this cost months and two blocked onboardings, and it was in the getting-started page the whole time.
