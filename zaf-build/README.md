...
| `shared_author_user_id` | you, if wanted | Numeric Zendesk user id of a dedicated shared user (for example "IBM via TSANet") that authors connector-written comments: inbound descriptions, receipts and mirrored partner notes. Empty = comments show under the connection user. Create it with an email on YOUR OWN domain, never the partner's, so notifications cannot route outside your org. An end user (no seat) or a light agent both work; comments stay private either way. A wrong id cannot break ticket creation — Zendesk silently falls back to the connection user — which is exactly why pre-flight resolves and shows the user's name |
...
- **No manifest property makes the sidebar fit its content.** A ZAF location takes `url`, `autoHide`, `autoLoad`, `flexible`, `signed` and `size`, and `flexible` governs *width*, only on `ticket_sidebar`. `flexible_height` sat in this manifest for many releases doing nothing and was removed in v1.0.63 (`tsanetgit/Zendesk_App#179`). Height comes only from `client.invoke('resize')`, which `main.js` calls in exactly one place, behind `fitPanelToContent()` and clamped by `PANEL_MIN_H`/`PANEL_MAX_H` so the panel cannot push your other sidebar apps out of reach. If you add a state that changes the panel's height, call `fitPanelToContent()` **after** its content is in the DOM — measuring before the render is why the old hardcoded heights existed.
