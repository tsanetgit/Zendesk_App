...
- **Server-side complement (optional, externally-hosted):** a GitHub Actions workflow (separate from this bundle, and from this repo's core setup — see [`GitHub_Actions_SLA_Monitor.md`](../GitHub_Actions_SLA_Monitor.md)) can tag tickets on SLA breach when no agent is online. It's optional convenience — TSANet enforces the acknowledgment SLA server-side regardless.
