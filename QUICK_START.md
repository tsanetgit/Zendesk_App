...
> **You must be a Zendesk administrator.** The nav bar screen is visible to any agent, so the app's own role check is a convenience gate rather than a security boundary. The boundary is server-side: Zendesk documents the ZIS registry endpoints as [Allowed for: Admins](https://developer.zendesk.com/api-reference/integration-services/registry/bundles/). That is Zendesk's documentation; TSANet has not tested an agent-role session against these endpoints (`tsanetgit/Zendesk_App#125`).
