...
| The integration log shows `Cannot start flow because the connection named "zendesk" does not exist` (or the same for `tsanet_oauth`), and every affected job ends `FAILED` | The flow bundle resolves two named connections, `zendesk` (Step 4a) and `tsanet_oauth` (Step 2c), and at least one does not exist in your integration container. The engine names only **one** missing connection per run, so two different errors seconds apart are usually one root cause: both are missing. Three causes, in order of likelihood. **First, the `verification_code` exchange was skipped**: creation is not complete until you GET the `redirect_url`, even for client credentials (see the row above). **Second, the connection was created under a different integration name**: connections are integration-scoped, and the integration name goes in the connection URL path, so if Step 1b forced you onto your own name, every connection call in Steps 2 and 4a must use it too. **Third, the connection was created under a different name**: the bundle resolves `zendesk` and `tsanet_oauth` exactly, case-sensitive. Re-run Steps 2b–2d and 4a with your integration name substituted in every URL path, then confirm each connection holds a live `access_token` (Step 2d). Jobs that failed this way are terminal and are **not replayed** after the fix: pull the affected cases into tickets with the app's inbound sync, or ask the partner to re-send |
