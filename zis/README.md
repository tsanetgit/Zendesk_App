...
| Integration name — **usually leave alone** | the bundle's top-level `name`, and **all 18** `zis:tsanet_connect:action\|flow:*` ARNs | File ships with `tsanet_connect`, which is also the default. Set `tsanet_integration_name` only if Zendesk refuses that name for your account: `POST /registry/tsanet_connect` returns `400 the integration: tsanet_connect is not available for upsert by this account`, which is a namespace collision, not a permissions problem (#174). The app rewrites the top-level name structurally and every ARN textually, and refuses to upload if any `zis:tsanet_connect:` survives |
