...
| What | Where | Note |
|---|---|---|
| Custom field IDs | `action_create_ticket`, `action_search_ticket`, and `flow_handle_ping`'s `TransformForUpdate` jq (the update body is built there and handed to `action_update_ticket` whole) | The three numeric IDs (TSANet Token / Status / Partner, plus Respond By in the update path), taken from **your** instance's fields |
| Shared author user — **optional** | `action_create_ticket`, `action_zd_finish_status`, `action_zd_finish_note_receipt`, `action_zd_finish_fail` — every comment the bundle writes | Numeric Zendesk user id (`1234567896`) of a dedicated shared user to author connector comments. Leave blank and the `author_id` keys are removed from the upload entirely; comments then show under the connection user. A nonexistent id does NOT fail the write — Zendesk silently attributes to the connection user instead (probed), which is why the app's pre-flight resolves the id and shows you the name |
...
| Integration name — **usually leave alone** | the bundle's top-level `name`, and **all 18** `zis:tsanet_connect:action\|flow:*` ARNs | File ships with `tsanet_connect`, which is also the default. Set `tsanet_integration_name` only if Zendesk refuses that name for your account: `POST /registry/tsanet_connect` returns `400 the integration: tsanet_connect is not available for upsert by this account`. That is by design, not a permissions problem: [Zendesk documents](https://developer.zendesk.com/documentation/integration-services/zis-tutorials/getting-started/building-your-first-zis-integration/) that "the name you choose has to be globally unique and can be up to 64 characters long", with the charset "lower-case letters (a-z), numbers, hyphens (-), and underscores (_)" — so a name registered by any Zendesk account is unavailable to every other one (#174). The app rewrites the top-level name structurally and every ARN textually, and refuses to upload if any `zis:tsanet_connect:` survives |
...
> **Renaming an instance that already deployed.** The old integration is not retired by the rename: its job specs stay installed and keep intercepting the same events, so both integrations act on every inbound collaboration and tickets get created twice. The app's pre-flight warns when the default `tsanet_connect` still has installed specs, and prints the `DELETE .../job_specs/install?job_spec_name=...` to retire them. A rename from one **custom** name to another is undetectable from the app, because it has no record of the previous value — retire the old specs by hand in that case. Separately, ZIS connections are integration-scoped, so `tsanet_oauth` and `zendesk` both have to be created again under the new integration; the bundle will deploy without them and then fail auth on every TSANet call.

> Validated end-to-end on Beta (`connect2.tsanet.net`): authenticated webhook deliveries return 200 and the flow creates Zendesk tickets. The per-instance set is field IDs, host, `engineerEmail`, the OAuth connection name, **and (only where forced) the integration name** — nothing else is environment-specific.
