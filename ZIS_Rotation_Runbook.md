...
wherever it is used, so handling matters as much as cadence.
...
around. Two of the three credentials here already work that way: the
inbound webhook credential is generated by ZIS inside your own instance,
and the Zendesk OAuth secret is regenerated in your own instance by the
rotation script. Neither ever crosses an organizational boundary. The
Entra client secret is the exception, since TSANet issues it, so the
guidance below applies mainly to that one.
